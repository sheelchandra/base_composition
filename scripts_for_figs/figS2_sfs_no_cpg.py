"""
Figure S2: derived allele frequency (DAF) spectrum by mutation type,
excluding CpG/TpG sites (a control checking whether the Figure 3 pattern
is driven by CpG hypermutability).

FigS2A - broken-y-axis grid of SNP density vs. DAF bin, one panel per
         mutation type, CpG/TpG sites excluded.
FigS2B - mean DAF per mutation type and population, faceted by frequency
         stratum, from bootstrap CIs computed on the CpG/TpG-excluded set.

Run with: python figS2_sfs_no_cpg.py (from inside scripts/)
"""
...
SUPP_DIR = "/Users/sheelchandra/Downloads/base_comp_github/base_comp_input_files/supp/"
FIG_DIR = "/Users/sheelchandra/Downloads/base_comp_github/base_comp_figs/supp_figs/"

# ===========================================================================
# dataset for population-specific DAF (incl. singletons), CpG/TpG excluded
# ===========================================================================
# Column order follows the file's own header (AFR, EUR, AMR, EAS, SAS).
#
# dtype="category" for MUTATION avoids materializing ~30M+ individual Python
# string objects (there are only 12 distinct mutation types) -- this is the
# difference between this multi-GB file loading in a couple GB of memory
# versus not fitting in memory at all on a modest machine. Reads the .gz
# directly -- pandas decompresses on the fly.
masterdf = pd.read_csv(
    SUPP_DIR + "all_chrs_polarized_no_CpG_TpG.txt.gz", sep="\t", header=0,
...
plot_sfs_broken_grid(
    ...
    save_path=FIG_DIR + "FigS2A.png",
)
print(f"Wrote FigS2A.png to {FIG_DIR}")

# ===========================================================================
# FigS2B
# ===========================================================================
results_df = pd.read_csv(SUPP_DIR + "no_CpG_bootstrap_av_DAF.txt", sep="\t", header=0)
...
plt.tight_layout()
plt.savefig(FIG_DIR + "FigS2B.png", dpi=300, bbox_inches="tight")
plt.close()

print(f"Wrote FigS2B.png to {FIG_DIR}")

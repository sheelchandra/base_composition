"""
Figure S1: bottleneck effect on base composition, excluding singletons.

Same as Figure 2, but the "All" variant set for each species excludes
singletons (see the *_excl_singl* input files). The "Common (MAF>=5%)"
set is unchanged from Figure 2, since common variants already exclude
singletons by construction.

Run with: python figS1_bottleneck_no_singletons.py (from inside scripts/)
"""
...
silk_all = load_props(SUPP_DIR + "silkworm_all_excl_singl_sums.txt")
...
silk_all["VariantSet"] = "All (excl. singletons)"
...
maize_all = load_props(SUPP_DIR + "maize_all_excl_singl_sums.txt")
...
maize_all["VariantSet"] = "All (excl. singletons)"
...
mouse_all = load_props(SUPP_DIR + "mouse_all_excl_singl_sums.txt")
...
mouse_all["VariantSet"] = "All (excl. singletons)"
...
human_all = load_human(SUPP_DIR + "nucleotide_proportions_all_excl_singl.txt")
...
human_all["VariantSet"] = "All (excl. singletons)"
...
plt.savefig(FIG_DIR + "FigS1.png", dpi=300)
plt.close()

print(f"Wrote FigS1.png to {FIG_DIR}")
